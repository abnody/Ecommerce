...
export default function AllOrders() {
  ...
  {isLoading?<Loading/>:<>
    {(orders).map((order,index) => 
      <div key={index} className="relative overflow-x-auto rounded-2xl  my-16">
        <div className='flex flex-wrap  lg:justify-between px-5 py-6 items-center bg-[#1a1a1a] text-white mb-2'>
            <i onClick={()=>toggle()} className={`fa-solid ${clicked?"fa-chevron-up":"fa-chevron-down"} fa-xl mx-2 px-3 py-5 rounded-lg cursor-pointer bg-gray-400 text-[#1a1a1a]`}></i>
          ...
          <table className={` ${clicked?'':'hidden'} w-full text-sm text-left rounded-2xl rtl:text-right text-[#1a1a1a]`}>
